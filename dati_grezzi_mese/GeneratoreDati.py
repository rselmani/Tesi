    ...
    for idx_t, (h_in, h_out) in enumerate(turni_orari):
        ...
        for idx_m, cod_m in enumerate(macchine):
            ...
            def scarica_contenitore(orario, fine_turno_senza_collaudo=False):
                global cod_box, tipo_box, pezzi_nel_box
                if pezzi_nel_box > 0 and cod_box is not None:
                    st_info = stampi[stato_macchine[cod_m]["stampo"]]
                    
                    # Taratura calibrata al ~3.2% di scarto per ottenere una resa del ~96.8%
                ...
                pezzi_nel_box, cod_box, tipo_box = 0, None, None
            ...
            while orologio < t_fine:
                ...
                min_turno = (orologio - t_inizio).total_seconds() / 60.0
                temp = 290.0 + (math.sin(min_turno / 25.0) * 1.4)
                press = 5.80 + (math.cos(min_turno / 12.0) * 0.12)
...
print("\nDati grezzi rigenerati con successo (Resa qualità ~96.8%)!")